...
include_once "./printPicW.php";
	...
	//	上传 图片加水印()		表单name:string , 允许类型array[]:string ， 保存路径string  是否多图片bool ，是否随机名bool，是否覆盖同名图片bool 是否添加水印bool 水印图path:string
function upPicPW($postName,$allowType,$saveDir="upload/",$isMany=0,$isRandName=0,$isCover=0,$isMark=0,$markPath=''){
	...
	if(!$isMany){
		...
		if($files['error']==0){
				...
				if(in_array($filetype,$allowType)){
					...
					if(move_uploaded_file($files["tmp_name"],$saveDir.iconv('utf-8','gbk',$filename).".".$filetype)){
						
						if(@getimagesize($saveDir.iconv('utf-8','gbk',$filename).".".$filetype)){
							echo "aa";
							if($isMark && isImage($markPath)){
								echo $saveDir.$filename.".".$filetype;
								echo printPicW($saveDir.$filename.".".$filetype,$markPath);// 添加水印
							}
